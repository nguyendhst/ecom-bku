...
type cart = {
    ...
    addNItem: (item: cartNewItem, quantity: number) => void;
    currency: string;
    cartCount: number;
...
const cartReducer: cartReducer = (state, action) => {
    switch (action.type) {
        ...
        case "ADD_N_ITEM": {
            console.log("ADD_N_ITEM");
            // if payload is undefined, return state
            if (!action.payload) return state;

            const itemToAdd: cartNewItem = action.payload;
            const quant = action.quantity ?? 1;
            // find the item in the cart
            const existingItem = state.cartDetails[itemToAdd.id];
            console.log("existingItem", existingItem);
            // if the item is already in the cart, update the quantity
            if (existingItem) {
                const updatedItem = {
                    ...existingItem,
                    quantity: existingItem.quantity + quant,
                    totalPrice:
                        existingItem.price * (existingItem.quantity + quant),
                    formattedTotalPrice: toVND(
                        existingItem.price * (existingItem.quantity + quant)
                    ),
                };
                ...
                return {
                    ...state,
                    cartCount: state.cartCount + quant,
                    totalPrice: state.totalPrice + existingItem.price * quant,
                    cartDetails: {
                        ...state.cartDetails,
                        [itemToAdd.id]: updatedItem,
                    },
            ...
            const newItem: cartEntry = {
                ...itemToAdd,
                quantity: quant,
                totalPrice: itemToAdd.price * quant,
                formattedTotalPrice: toVND(itemToAdd.price * quant),
                timeAdded: new Date().toISOString(),
            };
            return {
                ...state,
                cartCount: state.cartCount + quant,
                totalPrice: state.totalPrice + newItem.price * quant,
                cartDetails: {
                    ...state.cartDetails,
                    [itemToAdd.id]: newItem,
                },
...
export function ShoppingCartProvider({ children }: any) {
    ...
    const addNItem = (item: cartNewItem, quantity: number) => {
        console.log(item, state.cartDetails);
        // Add item to cartDetails and update cartCount and totalPrice
        dispatch({
            type: "ADD_N_ITEM",
